...
export default function AIFeatureCard({ 
  ...
  return (
    ...
    >
      <div 
        className={cn(
          "h-full rounded-2xl px-4 py-3.5 transition-[background-color,border-color,shadow] duration-200 ease-out relative overflow-hidden flex items-stretch gap-3.5 border",
          isActive 
      ...
      >
        {/* Active side indicator pill - always rendered to prevent DOM/layout recalculations */}
        <div 
          className={cn(
            "absolute left-0 top-0 bottom-0 w-[3px] bg-gradient-to-b from-emerald-500 to-teal-500 transition-opacity duration-200",
            isActive ? "opacity-100" : "opacity-0"
          )} 
        />

        {/* Icon - self-start pins it to top; base has border to prevent 2px shift */}
        <div className={cn(
          "p-2.5 rounded-xl flex-shrink-0 flex items-center justify-center self-start transition-[background-color,color,box-shadow,border-color] duration-200 shadow-sm border",
          isActive 
            ? "bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-emerald-500/20 border-transparent" 
            : "bg-zinc-50/80 text-zinc-500 border-zinc-200/55 group-hover:bg-emerald-500/10 group-hover:text-emerald-600 group-hover:border-emerald-500/10"
        )}>
          <Icon className="h-4.5 w-4.5 stroke-[2]" />
        </div>

        {/* Text - flex col stretch ensures space-between pushes demo button to bottom */}
        <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch space-y-1">
          ...
          {/* Reserved link container - keeps all card heights and text placements 100% identical */}
          <div className="pt-2 min-h-[22px] flex items-center">
            {launchPath && (
              <Link 
                to={launchPath}
                className={cn(
