...
import { AiProvider, ScanResult, ChatResult, PlannerResult } from '../interfaces/ai-provider.interface';
...
3. REAL-TIME DATA HONESTY:
   ...
   - If a real-time question is asked and verified live data cannot be retrieved from search grounding, state clearly in the user's latest message language that you do not currently have access to verified real-time data.
...
==================================================
1. STRICT PRIVATE DATA BOUNDARY: Search provides access to PUBLIC web info ONLY. It does NOT grant access to ZAMINAT's private server logs, Cloud Run logs, internal analytics, production telemetry, module latency, Supabase records, private user analytics, internal business metrics, or infrastructure state.
...
export class GoogleGeminiProvider implements AiProvider {
  ...
  private getApiKey(): string {
    ...
    if (!key || key === 'your-gemini-api-key-here' || key === 'your-api-key-here') {
  ...
  private logProviderError(operation: string, model: string, err: any) {
    const sanitized = {
      ...
      errorMessage: err?.message
        ? String(err.message).replace(/key=[^&\s]+/gi, 'key=[REDACTED]').replace(/Bearer\s+[a-zA-Z0-9_\-\.]+/gi, 'Bearer [REDACTED]')
  ...
  async scanWaste(dto: ScanDto): Promise<ScanResult> {
    ...
    try {
      ...
      this.logProviderError('scanWaste', GEMINI_SCAN_MODEL, err);
  ...
  async chatCoach(dto: ChatDto): Promise<ChatResult> {
    ...
    try {
      ...
      this.logProviderError('chatCoach', GEMINI_CHAT_MODEL, err);
      throw new HttpException('AI_PROVIDER_ERROR', HttpStatus.BAD_GATEWAY);
    }
  }
  ...
  async optimizePlanner(dto: PlannerDto): Promise<PlannerResult> {
    ...
    try {
      ...
      this.logProviderError('optimizePlanner', GEMINI_PLANNER_MODEL, err);
      throw new HttpException('AI_PROVIDER_ERROR', HttpStatus.BAD_GATEWAY);
    }
  }
}
